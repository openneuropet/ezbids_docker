...
test_hosts = {
    ...
    hostname: {"url": f"http://{hostname}:3000", "api": f"http://{hostname}:8082"},
}
...
test_hosts = {
    REMOTE_HOST: {
        "url": f"http://{REMOTE_HOST}:3000",
        "api": f"http://{REMOTE_HOST}:8082"
    }
}
...
def test_upload_flat_folder_of_dicoms(active_host, web_driver, session):
    ...
    print("="*80 + "\n", flush=True)
    print("NOTE: If you don't see the session URL above, run pytest with -s flag:", flush=True)
    ...
    for i in range(0, len(files_to_upload), batch_size):
        ...
        print(f"Uploading batch {i//batch_size + 1}/{(len(files_to_upload) + batch_size - 1)//batch_size} ({len(batch)} files)")
        sys.stdout.flush()
    ...
    print(f"{'✓' if steps.get('analyzed') else '✗'} Status 'analyzed - successfully run preprocess.sh'")
    sys.stdout.flush()

    # Individual assertions for each step
    assert steps.get("uploaded"), "Upload step failed - status 'uploaded' not reached"
    assert steps.get("preprocessing"), "Preprocessing step failed - status 'preprocessing' not reached"
